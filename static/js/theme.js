(function($) {
    ...
    $(document).ready(function() {
        $(".home-products-slider").owlCarousel({
            ...
            margin: 20,
            autoWidth: true,
            navText: ["<div class='hsowl-prev' aria-label='Previous'><i class='far fa-chevron-left'></i></div>","<div class='hsowl-next' aria-label='Next'><i class='far fa-chevron-right'></i></div>"],
            slideBy: 'page'
        });
        APP.init();
...
let all_icons = document.getElementsByClassName("loadicons")
let loadicons = Object.values(all_icons)
let delay_num = 0
loadicons.sort(function(a, b){return 0.5 - Math.random()});
loading=true
// while (loading === true) {
for (let icon = 0; icon < loadicons.length; icon++) {
    delay_num += 0.3
